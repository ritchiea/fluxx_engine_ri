class FluxxEngineCreateMultiElementValues < ActiveRecord::Migration
  def self.up
    ...
    execute "alter table multi_element_values add foreign key multi_element_values_group_id (multi_element_group_id) references multi_element_groups(id)" if connection.adapter_name =~ /mysql/i
  end
