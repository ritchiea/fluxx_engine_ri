class FluxxEngineCreateMultiElementChoices < ActiveRecord::Migration
  def self.up
    ...
    execute "alter table multi_element_choices add foreign key multi_element_choice_value_id (multi_element_value_id) references multi_element_values(id)" if connection.adapter_name =~ /mysql/i
  end
